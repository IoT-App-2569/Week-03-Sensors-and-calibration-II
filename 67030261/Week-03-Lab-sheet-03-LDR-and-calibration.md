...
![](../Pasted%20image%2020260713115357.png)
...
- ความคลาดเคลื่อนอยู่ในระดับที่ยอมรับได้หรือไม่
	อยู่ในระดับที่ยอมรับได้เนื่องจากค่าไม่คลาดเคลื่อนมากจนเกินไปเเละอาจจากมีเเสงหรือสัญญาณรบกวนจากถายนอกจึงทำให้ค่าไม่ตรง 100% เเต่ไม่ต่างเกิน 20% ยังถือว่ายอมรับได้
- สมการ Linear หรือ Polynomial เหมาะกว่ากัน
	Polynomial เหมาะมากกว่าเพราะ กราฟเป็นเส้นโค้งและการเพิ่มขึ้นไม่ได้เป็นเส้นตรงแบบ Linear ทำให้ Polynomial เหมาะสมกว่า

## **ส่วนที่ 8 — คำถามท้ายใบงาน**

1. ทำไมต้องใช้ Voltage Divider ในการอ่าน LDR?
	เนื่องจาก ต้องการเปลี่ยนแปลงความต้านทานของ LDR ให้เป็นแรงดันที่แปรผันตามความเข้มแสง ทำให้ esp สามารถอ่านค่าได้ ถ้าไม่ต่อ Voltage Divider จะไม่สามารถอ่านค่าได้เพราะ LDR จะมีความปล่อย แรงดัน ออกมาเเค่นิดเดียวจนทำให้ไม่สามารถอ่านค่าได้
2. ทำไมต้องเก็บข้อมูลหลายจุดก่อนทำ Calibration?
	เนื่องจากจะทำให้ค่าที่วัดได้มีค่าที่เป็นมาตรฐานเดียวกันทั้งหมด ถ้าเพิ่ม lDR หลายตัวค่าที่วัดได้ในบริเวณเดียวกัน มีค่าเท่ากันเเละสามารถใช้ในการวัดค่าเเสงในหลายๆที่เเละสามารถใช้ต่อในหน่วย LUX ที่เป็นหน่วยมาตราฐานได้ เเละ LDR มีกราฟที่ไม่เป็นเส้นตรง การวัดจุดต่ำสุดเเละสูงสุดจะบอกการฟจริงๆไม่ได้การวัดหลายๆรอบ จะทำกราฟถูกต้องเเละเเม่นยำ
3. ถ้า Noise มากกว่าสัญญาณจริง จะเกิดอะไรขึ้น?
	ถ้า Noise มีค่ามากกว่าสัญญาณจริง จะทำให้ค่าที่อ่านได้ แกว่งตลอดเวลาเเละวัดค่าจริงๆของเเสงบริเวณนั้นไม่ได้เลยค่าที่วัดได้ทั้งหมดจะเป็น Noise ทั้งหมด
4. ทำไมการวางตำแหน่งเซนเซอร์จึงสำคัญต่อ Signal Integrity?
	เพื่อป้องกันไม่ให้เซนเซอร์อยู่ใกล้จุดที่มี Noise มาก จะทำให้ ค่าที่วัดได้ไม่เเกว่งเเละการวางตำแหน่งที่เหมาะสมช่วยลดความยาวสายไฟ ทำให้แรงดันสัญญาณไม่สูญหายระหว่างเดินทาง
5. ถ้า ADC มี Resolution ต่ำ จะส่งผลอย่างไรต่อความแม่นยำ?
	จะทำให้ความคลาดเคลื่อนจากการแปลงสัญญาณ สูงขึ้น เพราะมีที่เก็บสัญญาณน้อยเเล้วค่าที่ได้จะไม่ค่อยละเอียดทำให้ค่าที่วัดออกมาไม่ค่อยเเม่นยำถ้าเเสงเปลี่ยนเเปลงนิดเดียวเเล้ว Resolution ต่ำ ค่าที่วัดได้อาจไม่เปลี่ยนเเปลงเลยก็ได้เพราะมันอบยู่ในช่วงเดียวกัน ทำให้ค่าที่วัดออกมาไม่เปลี่ยนเเล้วจะทำให้เอาไปใช้ต่อยากเเละสูญเสียความละเอียด
...
| 1.2 | อธิบายการทำงานภายใน LDR (โฟตอน → อิเล็กตรอน → conduction band) | 4                    | ยังไม่เข้าใจเรื่องสารที่ใช้โฟตอนจากแสงวิ่งมาชนเเล้วความต้านทานเปลี่ยน |
...
| 2.3 | เขียนโปรแกรมอ่านค่า ADC ได้ถูกต้อง  | 5                    |        |
...
| ข้อ | รายการ                             | คะแนน<br>(0–5 คะแนน) | เหตุผล                                                                                                      |
| --- | ---------------------------------- | -------------------- | ----------------------------------------------------------------------------------------------------------- |
| 3.1 | เก็บข้อมูล ADC อย่างน้อย 5–7 จุด   | 5                    |                                                                                                             |
| 3.2 | วัดค่า Lux จากมือถือได้ถูกต้อง     | 4                    | เนื่องจากแอปที่ใช้อาจจะไม่ได้เเม่นยำ เนื่องจากใช้กล้องในการวัดอาจไม่เเม่นยำเหมือนใช้เครื่องวัดที่ได้มาตรฐาน |
| 3.3 | จดข้อมูลเป็นตารางครบถ้วน           | 5                    |                                                                                                             |
| 3.4 | ควบคุมสภาพแสงให้คงที่ระหว่างการวัด | 5                    |                                                                                                             |
...
| 4.2 | เลือก Trendline เหมาะสม (Linear / Polynomial) | 5                    |        |
| 4.3 | อ่านสมการจากกราฟได้ถูกต้อง                    | 5                    |        |
| 4.4 | นำสมการไปใช้ในโค้ดได้                         | 5                    |        |
| 4.5 | วิเคราะห์ว่า Linear หรือ Polynomial เหมาะกว่า | 5                    |        |
### **ส่วนที่ 10.5 — การทดสอบผลลัพธ์และวิเคราะห์ (15 คะแนน)**

| ข้อ | รายการ                                   | คะแนน<br>(0–5 คะแนน) | เหตุผล                                                                                     |
| --- | ---------------------------------------- | -------------------- | ------------------------------------------------------------------------------------------ |
| 5.1 | ทดสอบจุดแสงใหม่ที่ไม่ได้ใช้ตอนเก็บข้อมูล | 5                    |                                                                                            |
| 5.2 | เปรียบเทียบ Lux มือถือ vs Lux จากโค้ด    | 4                    | ค่ามีการคลาดเคลื่อนนิดหน่อยอาจเกิดจากสัญญาณรบกวนหรือ เเสงรบกวนหรือเซนเซอร์ไม่ได้เเม่นยำมาก |
| 5.3 | วิเคราะห์ความคลาดเคลื่อนและสาเหตุได้     | 5                    |                                                                                            |
.................................................................

###  **คะแนนรวมทั้งหมด:** `___98___ / 100`
...
1. จุดไหนในใบงานที่คิดว่ายากที่สุด? 
	ตอบ การวัดเเสงโดยใช้มือถือเเละ LDR เนื่องจากวัดได้ยากค่ไม่คงที่ เเละไม่ได้ใช้เเสงจากถายนอกจริงเเต่เป็นการใช้เเฟชรจากมือถือ ทำให้วัดยากเเละจดค่าได้ยาก
    
2. ถ้าทำใบงานนี้อีกครั้ง จะปรับปรุงอะไร?
	หาเครื่องมืดวัดเเสงหรือแอปที่วัดได้เเม่นยำกว่านี้ เเละพยายามใช้เเสงจากสถานที่จริงมากกว่าการใช้เเฟชรจากมือถือ
    
3. ส่วนไหนที่คิดว่าตนเองเข้าใจดีมาก? 
	การต่อวงจรเนื่องจากมีการพัฒนาตนเองในด้านนี้มาก เเละเป็นสิ่งที่สนใจเเละชื่นชอบทำให้สนุกเเละสามารถเรียนรู้ได้เร็วกว่าเรื่องอื่นๆ