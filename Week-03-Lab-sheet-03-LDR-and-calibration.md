...
| ห้องปกติ | 2.32 kΩ  |   2.678 V                     |
| กลางแจ้ง | 0.6179 Ω   | 3.108 V                       |
...
```
ไมโครคอนโทรเลอร์ไม่สามารถอ่านค่า ความต้านทาน ได้โดยตรงอ่านได้แค่เฉพาะแรงดันไฟฟ้าวงจร voltage drivider ทำหน้าที่แปลงการเปลี่ยนแปลงความต้านทานของ ldr ให้กลายเป็นการเปลี่ยนแปลงแรงดันไฟฟ้าที่ ADC สามารถวัดได้
```
2. ทำไมต้องเก็บข้อมูลหลายจุดก่อนทำ Calibration?
```
เซนเซอร์ LDR ไม่ได้เปลี่ยนค่าเป็นเส้นตรงถ้าเราเก็บข้อมูลแค่ 2 จุดแล้วขีดเส้นตรงหากัน ค่าระหว่างทางที่ได้จะเพี้ยน การเก็บข้อมูลหลาย ๆ ระดับแสง ช่วยให้เราเห็นเส้นกราฟที่แท้จริงและหาความสัมพันธ์ได้แม่นยำขึ้น
```
3. ถ้า Noise มากกว่าสัญญาณจริง จะเกิดอะไรขึ้น?
```
ตัวเลขที่บอร์ดอ่านได้จะแกว่ง พอเอาเลขไปคำนวณ ค่าแสงที่ได้ก็จะเพี้ยนและใช้งานจริงไม่ได้
```
4. ทำไมการวางตำแหน่งเซนเซอร์จึงสำคัญต่อ Signal Integrity?
```
ถ้าเราใช้สายไฟต่อเซนเซอร์ยาวเกินไป ไฟอาจจะตกหรือถ้าลากสายไปใกล้ ๆ กับมอเตอร์หรือสายไฟบ้าน จะมีคลื่นรบกวนแทรกเข้ามาในสาย ทำให้แรงดันไฟฟ้าที่จะส่งไปเข้าบอร์ดผิดเพี้ยน ค่าแสงที่อ่านได้ก็จะไม่ตรงกับความเป็นจริง
```
5. ถ้า ADC มี Resolution ต่ำ จะส่งผลอย่างไรต่อความแม่นยำ?
```
แสงเปลี่ยนไปแค่นิดเดียว บอร์ดจะมองไม่ออกและแยกความแตกต่างไม่ได้ ทำให้ค่าแสงที่อ่านออกมาดูหยาบ และตัวเลขกระโดดข้ามไปข้ามมา ความแม่นยำจึงลดลง
```
...
| --- | -------------------------------------------------------------- | -------------------- | ------ |
| 1.1 | อธิบายหลักการทำงานของ LDR ได้ถูกต้อง                           |       3               |เข้าใจภาพรวมว่าแสงมีผลต่อความต้านทาน แต่อาจจะยังอธิบายเชิงทฤษฎีได้ไม่ครบทุกประเด็น        |
| 1.2 | อธิบายการทำงานภายใน LDR (โฟตอน → อิเล็กตรอน → conduction band) |      1                |ไม่ค่อยเข้าใจกระบวนการทำงานเชิงลึกในระดับอนุภาค        |
| 1.3 | เข้าใจว่าความสัมพันธ์ R vs Lux ไม่เป็นเส้นตรง                  |             2         |   ทราบว่าไม่เป็นเส้นตรง แต่อาจจะยังสับสนเรื่องพฤติกรรมเชิงคณิตศาสตร์ของกราฟ     |
| 1.4 | เข้าใจเหตุผลที่ต้องใช้ Voltage Divider                         |          4            |   เข้าใจชัดเจนว่าบอร์ดรับได้แค่แรงดัน จึงต้องใช้วงจรนี้แปลงค่า R ให้เป็น V เพื่อให้อ่านค่าได้     |

### **ส่วนที่ 10.2 — การต่อวงจรและอ่านค่า ADC (20 คะแนน)**

| ข้อ | รายการ                              | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ----------------------------------- | -------------------- | ------ |
| 2.1 | ต่อวงจร Voltage Divider ได้ถูกต้อง  |           5           | สามารถต่อวงจรฮาร์ดแวร์จริงและอ่านค่าการเปลี่ยนแปลงได้สำเร็จ       |
| 2.2 | เลือกค่า R คงที่เหมาะสม (เช่น 10kΩ) |            5          | เลือกใช้อุปกรณ์ได้ถูกต้องตามสเปกที่กำหนดในใบงาน       |
| 2.3 | เขียนโปรแกรมอ่านค่า ADC ได้ถูกต้อง  |            4          | สามารถเขียนโค้ดและคอมไพล์ผ่านโดยไม่มี Error มั้ง       |
| 2.4 | ใช้ ESP‑IDF ADC + Calibration ได้   |          5            |  นำฟังก์ชันของ ESP-IDF มาปรับใช้กับเซนเซอร์ LDR ได้อย่างสมบูรณ์      |

### **ส่วนที่ 10.3 — การเก็บข้อมูลจริง (20 คะแนน)**

| ข้อ | รายการ                             | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ---------------------------------- | -------------------- | ------ |
| 3.1 | เก็บข้อมูล ADC อย่างน้อย 5–7 จุด   |           5           | เก็บจุดข้อมูลได้ครบถ้วนตามช่วงแสงที่แตกต่างกัน       |
| 3.2 | วัดค่า Lux จากมือถือได้ถูกต้อง     |             5         |  ใช้แอปพลิเคชันอ้างอิงค่าแสงได้สอดคล้องกับสภาพแวดล้อม      |
| 3.3 | จดข้อมูลเป็นตารางครบถ้วน           |           5           |บันทึกข้อมูลและจัดระเบียบตารางเพื่อนำไปกราฟได้เรียบร้อย       |
| 3.4 | ควบคุมสภาพแสงให้คงที่ระหว่างการวัด |             5        |พยายามควบคุมปัจจัยภายนอกเพื่อให้ค่าที่อ่านได้นิ่งที่สุด        |
...
| ข้อ | รายการ                                        | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | --------------------------------------------- | -------------------- | ------ |
| 4.1 | สร้างกราฟ Scatter ADC vs Lux ได้              |         5             | นำข้อมูลมาพล็อตลงในโปรแกรม (Excel/Google Sheet) ได้ถูกต้อง       |
| 4.2 | เลือก Trendline เหมาะสม (Linear / Polynomial) |         4             |   แสงสว่างเหมาะกับมืดๆไม่ค่อยโอเค      |
| 4.3 | อ่านสมการจากกราฟได้ถูกต้อง                    |           5           |   ดึงสมการคณิตศาสตร์จากกราฟมาใช้งานได้     |
| 4.4 | นำสมการไปใช้ในโค้ดได้                         |          5            |  สามารถจัดการเรื่องชนิดของตัวแปรและการคำนวณทศนิยมในโค้ดได้สำเร็จ      |
| 4.5 | วิเคราะห์ว่า Linear หรือ Polynomial เหมาะกว่า |              4        | มองเห็นข้อจำกัดของสมการที่เลือกใช้เมื่อเจอสภาพแสงแบบสุดขั้ว (มืดมาก/สว่างมาก)       |
### **ส่วนที่ 10.5 — การทดสอบผลลัพธ์และวิเคราะห์ (15 คะแนน)**

| ข้อ | รายการ                                   | คะแนน<br>(0–5 คะแนน) | เหตุผล |
| --- | ---------------------------------------- | -------------------- | ------ |
| 5.1 | ทดสอบจุดแสงใหม่ที่ไม่ได้ใช้ตอนเก็บข้อมูล |             5         | ทำการสุ่มสภาพแสงใหม่เพื่อทดสอบการทำงานของระบบ       |
| 5.2 | เปรียบเทียบ Lux มือถือ vs Lux จากโค้ด    |          5            | สามารถนำผลลัพธ์มาเปรียบเทียบกันได้ชัดเจน       |
| 5.3 | วิเคราะห์ความคลาดเคลื่อนและสาเหตุได้     |            5          | เข้าใจว่าความคลาดเคลื่อนเกิดจากสมการ หรือการเก็บจุดข้อมูลที่อาจจะยังน้อยเกินไป       |
.................................................................

###  **คะแนนรวมทั้งหมด:** `87 / 100`
...
```
ยากที่การจัดการชนิดของตัวแปร (Data Types) และการคำนวณเลขทศนิยมในโปรแกรม เนื่องจากหากประกาศตัวแปรหรือคำนวณผิดพลาด อาจทำให้ค่าทศนิยมสูญหาย ส่งผลให้ผลลัพธ์ที่แปลงจากสมการ Calibration คลาดเคลื่อนได้
```
    
2. ถ้าทำใบงานนี้อีกครั้ง จะปรับปรุงอะไร?
```
ปรับปรุงโดยการเพิ่มจำนวนจุดในการเก็บข้อมูล (Data Points) ให้มากขึ้นในหลากหลายระดับแสง เพื่อให้กราฟแสดงความสัมพันธ์มีความละเอียด ซึ่งจะช่วยให้สร้างสมการ Calibration ได้แม่นยำและครอบคลุมช่วงสภาพแสงได้ดีกว่าเดิม
```
    
3. ส่วนไหนที่คิดว่าตนเองเข้าใจดีมาก?
```
เข้าใจหลักการทำงานของวงจร Voltage Divider ร่วมกับ LDR อย่างชัดเจน เนื่องจากการลงมือปฏิบัติจริงทำให้เห็นภาพว่า เมื่อความเข้มแสงมากขึ้น (ความต้านทาน LDR ลดลง) แรงดันไฟฟ้าจะเพิ่มขึ้น และในทางกลับกัน เมื่อแสงน้อยลง แรงดันไฟฟ้าก็จะลดลงตรงตามทฤษฎี
```
