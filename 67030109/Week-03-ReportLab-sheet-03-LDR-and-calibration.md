...
![alt text](<Pasted image 20260720023944.png>)
...
**ยอมรับไม่ได้เลย** เพราะความคลาดเคลื่อนสูงเกินไปมาก (235% และ 1,762%) นำไปใช้งานจริงไม่ได้
- สมการ Linear หรือ Polynomial เหมาะกว่ากัน
**ยังสรุปไม่ได้** เพราะข้อมูลดิบที่ได้จากเซนเซอร์มีความผิดปกติ (สลับกันระหว่างห้องเรียนกับหน้าต่าง) ค่าที่วัดได้จากมือถือสวนทางกับค่า ADC แนะนำให้ทดลองและเก็บข้อมูลใหม่อีกรอบครับ

## **ส่วนที่ 8 — คำถามท้ายใบงาน**

1. ทำไมต้องใช้ Voltage Divider ในการอ่าน LDR?
LDR เป็นตัวต้านทานที่เปลี่ยนค่าตามแสง แต่ ADC อ่านค่าได้เฉพาะ "แรงดัน" ไม่ใช่ "ความต้านทาน" จึงต้องต่อ LDR ร่วมกับตัวต้านทานคงที่แบบอนุกรม (voltage divider) เพื่อแปลงการเปลี่ยนแปลงความต้านทานให้กลายเป็นแรงดันที่เปลี่ยนแปลงตามไปด้วย
2. ทำไมต้องเก็บข้อมูลหลายจุดก่อนทำ Calibration?
เพราะความสัมพันธ์ระหว่างค่าที่อ่านได้กับค่าจริง (เช่น แสง, อุณหภูมิ) มักไม่เป็นเส้นตรงตลอดช่วง การเก็บหลายจุดช่วยให้เห็นแนวโน้ม/สมการที่แม่นยำกว่าการเดาจากจุดเดียว ลด error จากค่าผิดปกติ (outlier) และครอบคลุมช่วงการทำงานจริง
3. ถ้า Noise มากกว่าสัญญาณจริง จะเกิดอะไรขึ้น?
**ถ้า Noise มากกว่าสัญญาณจริง จะเกิดอะไรขึ้น?**  
สัญญาณที่วัดได้จะไม่น่าเชื่อถือ อ่านค่าผิดเพี้ยน กระโดดไปมา (jitter) ระบบอาจตัดสินใจผิดพลาด เช่น ตรวจจับเหตุการณ์ที่ไม่ได้เกิดขึ้นจริง (false trigger) หรือมองข้ามการเปลี่ยนแปลงที่แท้จริงไป
4. ทำไมการวางตำแหน่งเซนเซอร์จึงสำคัญต่อ Signal Integrity?
**ทำไมการวางตำแหน่งเซนเซอร์จึงสำคัญต่อ Signal Integrity?**  
ตำแหน่งที่ไม่เหมาะสม (ใกล้แหล่งรบกวน สายไฟยาวเกินไป ระยะห่างจากแหล่งสัญญาณที่ต้องการวัดไม่เหมาะสม) จะทำให้เกิด noise เพิ่ม, สัญญาณลดทอน (attenuation), หรือรับสัญญาณที่ไม่ตรงกับสิ่งที่ต้องการวัดจริง ส่งผลให้ค่าที่อ่านได้คลาดเคลื่อน
5. ถ้า ADC มี Resolution ต่ำ จะส่งผลอย่างไรต่อความแม่นยำ?
ADC จะแบ่งช่วงแรงดันเป็นขั้นห่างๆ (step ใหญ่) ทำให้ไม่สามารถแยกความแตกต่างของสัญญาณเล็กๆ ได้ เกิด quantization error สูง ความละเอียดของข้อมูลต่ำ และการเปลี่ยนแปลงเล็กน้อยของสัญญาณจริงอาจไม่ถูกตรวจจับเลย
...
| --- | -------------------------------------------------------------- | -------------------- | ----------------------------------------------------------------------- |
| 1.1 | อธิบายหลักการทำงานของ LDR ได้ถูกต้อง                           | 5                    | ตัวต้านทานเปลี่ยนค่าตามแสง แสงมาก  R ลด                                 |
| 1.2 | อธิบายการทำงานภายใน LDR (โฟตอน → อิเล็กตรอน → conduction band) | 3                    | โฟตอนกระตุ้นอิเล็กตรอนขึ้น conduction band ทำให้ตัวพาประจุเพิ่ม R จึงลด |
...
| ข้อ | รายการ                              | คะแนน<br>(0–5 คะแนน) | เหตุผล                                                      |
| --- | ----------------------------------- | -------------------- | ----------------------------------------------------------- |
| 2.1 | ต่อวงจร Voltage Divider ได้ถูกต้อง  | 5                    | ต่อ LDR อนุกรมกับ R คงที่ จุดกลางเข้า ADC ถูกต้อง           |
| 2.2 | เลือกค่า R คงที่เหมาะสม (เช่น 10kΩ) | 5                    | เลือก R ใกล้เคียงค่ากลางของ LDR เพื่อความไวสูงสุด           |
| 2.3 | เขียนโปรแกรมอ่านค่า ADC ได้ถูกต้อง  | 3                    | โค้ดอ่านค่า ADC ได้ต่อเนื่อง ถูก pin ตรงกับแสงจริง          |
| 2.4 | ใช้ ESP‑IDF ADC + Calibration ได้   | 5                    | ใช้ ADC calibration ของ ESP-IDF แปลง raw เป็น mV แม่นยำขึ้น |

### **ส่วนที่ 10.3 — การเก็บข้อมูลจริง (20 คะแนน)**

| ข้อ | รายการ                             | คะแนน<br>(0–5 คะแนน) | เหตุผล                                             |
| --- | ---------------------------------- | -------------------- | -------------------------------------------------- |
| 3.1 | เก็บข้อมูล ADC อย่างน้อย 5–7 จุด   | 5                    | เก็บ 5–7 จุด เพื่อให้เห็นแนวโน้มชัดพอ fit สมการได้ |
| 3.2 | วัดค่า Lux จากมือถือได้ถูกต้อง     | 4                    | ใช้แอปมือถือวัด Lux ตำแหน่งเดียวกับเซนเซอร์        |
| 3.3 | จดข้อมูลเป็นตารางครบถ้วน           | 5                    | จดตาราง (ADC, Lux) ครบ ไม่ขาดหาย                   |
| 3.4 | ควบคุมสภาพแสงให้คงที่ระหว่างการวัด | 5                    | คุมแสงให้นิ่งระหว่างวัด ลด noise                   |
...
| ข้อ | รายการ                                   | คะแนน<br>(0–5 คะแนน) | เหตุผล                                                                        |
| --- | ---------------------------------------- | -------------------- | ----------------------------------------------------------------------------- |
| 5.1 | ทดสอบจุดแสงใหม่ที่ไม่ได้ใช้ตอนเก็บข้อมูล | 5                    | ทดสอบจุดแสงใหม่ที่ไม่เคยใช้ตอน calibrate                                      |
| 5.2 | เปรียบเทียบ Lux มือถือ vs Lux จากโค้ด    | 5                    | เทียบค่า Lux จากโค้ดกับจากมือถือ                                              |
| 5.3 | วิเคราะห์ความคลาดเคลื่อนและสาเหตุได้     | 5                    | คำนวณ %error และอธิบายสาเหตุ (ADC resolution, noise, ความคลาดเคลื่อนของสมการ) |
.................................................................

###  **คะแนนรวมทั้งหมด:** 91 / 100`

###  **คำถามสะท้อนความเข้าใจ (Reflection)**

ให้นักศึกษาเขียนตอบสั้น ๆ

1. จุดไหนในใบงานที่คิดว่ายากที่สุด? การคุมแสงให้อ่านค่าได้นิ่งๆ เพื่อเอาไปพล็อตกราฟครับ เพราะแสงในห้องจริงๆ มันชอบแกว่ง ขยับนิดเดียวค่า ADC ก็เปลี่ยนแล้ว

2. ถ้าทำใบงานนี้อีกครั้ง จะปรับปรุงอะไร? อยากหาพวกกล่องหรือโคมไฟมาคุมทิศทางแสงตอนเก็บข้อมูลให้ชัวร์กว่านี้ครับ จะได้ลด และสลับตำแหน่งวัดแล้วค่าไม่เยอะเกินไป

3. ส่วนไหนที่คิดว่าตนเองเข้าใจดีมาก? เรื่องการต่อวงจร Voltage Divider การได้อ่านค่าของ ADC
