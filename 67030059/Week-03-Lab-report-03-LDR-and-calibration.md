...
| -------- | ------------- | ---------------------- |
| มืดสนิท  | 200 kΩ – 2 MΩ | 1.903 V                |
| ห้องปกติ | 10 kΩ – 50 kΩ | 2.699 V                |
| ใต้โคมไฟ | 2 kΩ – 10 kΩ  | 0.4184 v               |
| กลางแจ้ง | 200 Ω – 2 kΩ  |                        |
...
ค่าที่ทดสอบได้
![](../Pasted%20image%2020260714123007.png)
...
2. ตอบเพราะ LDR เป็นตัวต้านทานที่เปลี่ยนค่าความต้านทานตามความเข้มแสง จึงต้องต่อร่วมกับตัวต้านทานคงที่เป็นวงจร **Voltage Divider** เพื่อแปลงค่าความต้านทานให้เป็นแรงดันไฟฟ้า ซึ่งไมโครคอนโทรลเลอร์สามารถอ่านค่าได้ผ่านขา ADC
3. ทำไมต้องเก็บข้อมูลหลายจุดก่อนทำ Calibration?
4. ตอบ เพราะการวัดเพียงครั้งเดียวอาจมีความคลาดเคลื่อนจาก Noise หรือสภาพแวดล้อม การเก็บข้อมูลหลายจุดช่วยให้เห็นแนวโน้มของข้อมูล คำนวณค่าที่เหมาะสม และทำให้การ Calibration มีความแม่นยำและเชื่อถือได้มากขึ้น
5. ถ้า Noise มากกว่าสัญญาณจริง จะเกิดอะไรขึ้น?
6. ตอบ จะทำให้ค่าที่วัดได้ไม่ตรงกับสัญญาณจริง ข้อมูลมีความผันผวน ระบบอาจตีความผิดพลาด ส่งผลให้การทำงานของเซนเซอร์หรือการตัดสินใจของระบบไม่มีความแม่นยำ
7. ทำไมการวางตำแหน่งเซนเซอร์จึงสำคัญต่อ Signal Integrity?
8. ตอบ เพราะตำแหน่งของเซนเซอร์มีผลต่อคุณภาพของสัญญาณ หากวางในตำแหน่งที่ไม่เหมาะสม เช่น ใกล้แหล่งรบกวนทางไฟฟ้าหรือรับแสงไม่ตรงกับที่ต้องการ อาจทำให้เกิดสัญญาณรบกวน (Noise) หรือค่าที่วัดคลาดเคลื่อน ส่งผลให้
9. ถ้า ADC มี Resolution ต่ำ จะส่งผลอย่างไรต่อความแม่นยำ?
10. ตอบ ADC ที่มี Resolution ต่ำจะแบ่งระดับแรงดันได้เป็นจำนวนขั้นน้อย ทำให้การเปลี่ยนแปลงของสัญญาณเล็ก ๆ ไม่สามารถตรวจจับได้ ค่าที่อ่านได้จึงหยาบกว่าและมีความแม่นยำน้อยลงเมื่อเทียบกับ ADC ที่มี Resolution สูง
...
| --- | -------------------------------------------------------------- | -------------------- | ------------------------------------------------------- |
| 1.1 | อธิบายหลักการทำงานของ LDR ได้ถูกต้อง                           | 5                    | อธิบายหลักการทำงานได้ถูกต้องและครบถ้วน                  |
| 1.2 | อธิบายการทำงานภายใน LDR (โฟตอน → อิเล็กตรอน → conduction band) | 5                    | เข้าใจการเกิดอิเล็กตรอนและการนำไฟฟ้าภายใน LDR           |
| 1.3 | เข้าใจว่าความสัมพันธ์ R vs Lux ไม่เป็นเส้นตรง                  | 5                    | เข้าใจความสัมพันธ์และเลือกใช้กราฟ Polynomial ได้เหมาะสม |
| 1.4 | เข้าใจเหตุผลที่ต้องใช้ Voltage Divider                         | 5                    | อธิบายเหตุผลและการนำไปใช้งานได้ถูกต้อง                  |

### **ส่วนที่ 10.2 — การต่อวงจรและอ่านค่า ADC (20 คะแนน)**

| ข้อ | รายการ                              | คะแนน<br>(0–5 คะแนน) | เหตุผล                                            |
| --- | ----------------------------------- | -------------------- | ------------------------------------------------- |
| 2.1 | ต่อวงจร Voltage Divider ได้ถูกต้อง  | 5                    | ต่อวงจรได้ถูกต้องตามหลักการ                       |
| 2.2 | เลือกค่า R คงที่เหมาะสม (เช่น 10kΩ) | 5                    | เลือกใช้ตัวต้านทาน 10 kΩ ได้เหมาะสม               |
| 2.3 | เขียนโปรแกรมอ่านค่า ADC ได้ถูกต้อง  | 5                    | อ่านค่า ADC และแสดงผลได้ถูกต้อง                   |
| 2.4 | ใช้ ESP‑IDF ADC + Calibration ได้   | 5                    | สามารถใช้งานและประยุกต์ใช้ Calibration ได้ถูกต้อง |

### **ส่วนที่ 10.3 — การเก็บข้อมูลจริง (20 คะแนน)**

| ข้อ | รายการ                             | คะแนน<br>(0–5 คะแนน) | เหตุผล                                                      |
| --- | ---------------------------------- | -------------------- | ----------------------------------------------------------- |
| 3.1 | เก็บข้อมูล ADC อย่างน้อย 5–7 จุด   | 5                    | เก็บข้อมูลครบตามที่กำหนด                                    |
| 3.2 | วัดค่า Lux จากมือถือได้ถูกต้อง     | 5                    | วัดและบันทึกข้อมูลได้ครบถ้วน                                |
| 3.3 | จดข้อมูลเป็นตารางครบถ้วน           | 5                    | จัดทำตารางข้อมูลเป็นระเบียบและครบถ้วน                       |
| 3.4 | ควบคุมสภาพแสงให้คงที่ระหว่างการวัด | 4                    | ควบคุมสภาพแสงได้ดี แต่ยังอาจมีปัจจัยแวดล้อมที่ส่งผลเล็กน้อย |
...
| 4.2 | เลือก Trendline เหมาะสม (Linear / Polynomial) | 5                    | เลือก Polynomial ได้เหมาะสมกับข้อมูล                     |
| 4.3 | อ่านสมการจากกราฟได้ถูกต้อง                    | 5                    | อ่านสมการและนำไปใช้ได้ถูกต้อง                            |
| 4.4 | นำสมการไปใช้ในโค้ดได้                         | 5                    | นำสมการไปประยุกต์ใช้ในโปรแกรมได้สำเร็จ                   |
| 4.5 | วิเคราะห์ว่า Linear หรือ Polynomial เหมาะกว่า | 4                    | อธิบายเหตุผลได้ดี แต่ยังสามารถวิเคราะห์เชิงลึกได้มากขึ้น |
### **ส่วนที่ 10.5 — การทดสอบผลลัพธ์และวิเคราะห์ (15 คะแนน)**

| ข้อ | รายการ                                   | คะแนน<br>(0–5 คะแนน) | เหตุผล                                                                                  |
| --- | ---------------------------------------- | -------------------- | --------------------------------------------------------------------------------------- |
| 5.1 | ทดสอบจุดแสงใหม่ที่ไม่ได้ใช้ตอนเก็บข้อมูล | 5                    | มีการทดสอบเพิ่มเติมนอกชุดข้อมูลเดิม                                                     |
| 5.2 | เปรียบเทียบ Lux มือถือ vs Lux จากโค้ด    | 5                    | เปรียบเทียบผลการวัดได้ครบถ้วน                                                           |
| 5.3 | วิเคราะห์ความคลาดเคลื่อนและสาเหตุได้     | 4                    | วิเคราะห์สาเหตุได้ แต่ยังสามารถอธิบายผลกระทบของ Noise และปัจจัยแวดล้อมได้ละเอียดกว่านี้ |
|     |                                          |                      |                                                                                         |
.................................................................
...
2. ตอบ การสร้างสมการ Calibration และเลือก Trendline ที่เหมาะสม เนื่องจากต้องวิเคราะห์ข้อมูลจากกราฟและเปรียบเทียบความแม่นยำของแต่ละสมการ
3. ถ้าทำใบงานนี้อีกครั้ง จะปรับปรุงอะไร? 
4. ตอบ จะเพิ่มจำนวนจุดข้อมูลในการเก็บค่าและควบคุมสภาพแสงให้คงที่มากขึ้น เพื่อลดความคลาดเคลื่อนของผลการวัด
5. ส่วนไหนที่คิดว่าตนเองเข้าใจดีมาก? 
6. ตอบ การต่อวงจร Voltage Divider การอ่านค่า ADC ของ ESP32 และการนำสมการ Calibration ไปใช้คำนวณค่า Lux ในโปรแกรมได้อย่างถูกต้อง
